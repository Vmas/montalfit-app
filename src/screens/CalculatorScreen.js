...
export default function CalculatorScreen({ navigation }) {
  ...
  const [sexo, setSexo] = useState('hombre'); // 'hombre' o 'mujer'
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.row}>
        <TouchableOpacity 
          style={[styles.miniBtn, sexo === 'hombre' && styles.active]} 
          onPress={() => setSexo('hombre')}
        >
          <Text style={styles.btnText}>HOMBRE</Text>
        </TouchableOpacity>
        <TouchableOpacity 
          style={[styles.miniBtn, sexo === 'mujer' && styles.active]} 
      ...
      <View style={styles.row}>
        <TouchableOpacity style={[styles.miniBtn, actividad === 1.2 && styles.active]} onPress={() => setActividad(1.2)}>
          <Text style={styles.btnText}>Poca</Text>
        </TouchableOpacity>
      ...
      <View style={styles.row}>
        <TouchableOpacity style={[styles.miniBtn, objetivo === -500 && styles.activeDeficit]} onPress={() => setObjetivo(-500)}>
          <Text style={styles.btnText}>Perder</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity style={[styles.miniBtn, objetivo === 400 && styles.activeSuperavit]} onPress={() => setObjetivo(400)}>
          <Text style={styles.btnText}>Ganar</Text>
        </TouchableOpacity>
      </View>

      <TouchableOpacity style={styles.buttonMain} onPress={calcularTMB}>
        <Text style={styles.buttonText}>CALCULAR Y GUARDAR</Text>
      </TouchableOpacity>

      {resultado && (
        <View style={styles.resultCard}>
          <Text style={styles.resultLabel}>Meta Diaria:</Text>
          ...
          <TouchableOpacity style={styles.btnNext} onPress={() => navigation.navigate('Diary')}>
            <Text style={styles.buttonText}>IR AL DIARIO →</Text>
          </TouchableOpacity>
        </View>
      )}
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  miniBtn: { backgroundColor: 'rgba(255,255,255,0.1)', padding: 12, borderRadius: 8, width: '31%', alignItems: 'center' },
  active: { backgroundColor: '#28A745' },
  activeDeficit: { backgroundColor: '#FFC107' }, // Amarillo para alerta de déficit
  activeSuperavit: { backgroundColor: '#007BFF' }, // Azul para ganar masa
  ...
  resultCard: { marginTop: 25, alignItems: 'center', padding: 20, backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 20, borderWidth: 1, borderColor: '#28A745' },
